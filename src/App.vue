...
export default {
  ...
  methods: {
    ...
    openMoviePopup(id, newMoviePopup){
      if(newMoviePopup){
        constants.backTitle = document.title;
      }
      constants.createMoviePopup = newMoviePopup;
    ...
    onHistoryState(e){
      constants.moviePopupOnHistory = e.state ? e.state.hasOwnProperty('popup') : false;
      if(!constants.moviePopupOnHistory){
        this.moviePopupIsVisible = false;
        document.title = constants.backTitle;
      }
    },
    changeHistoryState(){
      if(history.state && history.state.popup){
        ...
        history.replaceState(newState , null, constants.moviePath);
      }
    },
    // Router After Leave
    afterLeave(){
