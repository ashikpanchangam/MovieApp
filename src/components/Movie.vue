...
import img from '../directives/image.js'
...
export default {
  ...
  methods: {
    fetchMovie(id){
      axios.get(`https://api.themoviedb.org/3/movie/${id}?api_key=${constants.apiKey}&language=en-US`)
      .then(function(resp){
          let movie = resp.data;
          this.movie = movie;
          this.poster();
          ...
          // For maintining the state after the user clicks on the close icon
          if(constants.createMoviePopup){
            constants.moviePath = '/movie/' + id;
            history.pushState({ popup: true }, null, constants.moviePath);
            constants.createMoviePopup = false;
          }
          
          document.title = this.movie.title + constants.pageTitlePostfix;
