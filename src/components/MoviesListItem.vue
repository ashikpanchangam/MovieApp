  ...
  <li class="movies-item">
    <a class="movies-item__link" :class="{'no-image': noImage}" :href="'/movie/' + movie.id" @click.prevent="openMoviePopup(movie.id, true)">
      <figure class="movies-item__poster">
        <img v-if="!noImage" class="movies-item__img" src="~assets/placeholder.png" :title="'Avg Rating: '+movie.vote_average+'/10'" v-img="poster()">
        <img v-if="noImage" class="movies-item__img is-loaded" src="~assets/no-image.png">
      </figure>
      <div class="movies-item__content">
        <p class="movies-item__title" style="color:#01d277"><strong>{{ movie.title }}</strong></p>
      </div>
    </a>
  </li>
</template>
